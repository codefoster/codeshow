    ...
    function loadDemosAsync() {
        ...
        return new WinJS.Promise(function (c, e, p) {
            ...
            Windows.ApplicationModel.Package.current.installedLocation.getFolderAsync("demos")
                .then(function(demosFolder) {
                    return getChildFoldersAsync(demosFolder);
                })
                .then(function(results) {
                    
                });

        //recurse all folders and store them flat
        //for each folder fetch the files and add to result

        //should receive a storagefolder and return a flat array of all folders/subfolders
        function getChildFoldersAsync(folder) {
            var results = [];
            folder.getFoldersAsync()
                .then(function(ff) {
                    ff.forEach(function(f) {
                        results.push({ name: f.name }); //add the folder
                        getChildFoldersAsync(f);
                    });
                    c(results);                        
                });
        ...
        return new WinJS.Promise(function (c, e, p) {
            Windows.ApplicationModel.Package.current.installedLocation.getFolderAsync("demos")
                .then(function (demosFolder) {
                    processFolder(demosFolder);
                    //return demosFolder.getFoldersAsync();
                })
